...
fn setup(mut commands: Commands, mut texture_atlases: ResMut<Assets<TextureAtlas>>, asset_server: Res<AssetServer>, mut materials: ResMut<Assets<ColorMaterial>>) {
    //Map Creation
    ...
    commands.spawn()
        ...
        .with_children(|parent| {
            parent.spawn_bundle(SpriteSheetBundle {
                texture_atlas: texture_atlases.add(TextureAtlas::from_grid(asset_server.load("earth2.png"), Vec2::splat(100.), 50, 50)),
                transform: Transform {
    ...
    let names = vec!["background_4.png", "background_3.png", "background_2.png", "background_1.png"];
    for i in 1..5 {
...
fn parallax_system(
    ...
    if let Ok(cam_trans) = cam_query.single() {
        for mut trans in query.iter_mut() {
            trans.translation.x = -cam_trans.translation.x * (0.002 * (trans.translation.z - BACKGROUND_Z));
            trans.translation.y = -cam_trans.translation.y * (0.001 * (trans.translation.z - BACKGROUND_Z));
        }
    }
}
